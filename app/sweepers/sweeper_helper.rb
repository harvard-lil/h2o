module SweeperHelper
  ...
  def expire_question_instance(record)
    ...
    Question::POSSIBLE_SORTS.keys.each do |sort|
      expire_fragment("question-instance-index-#{record.question_instance_id}-true-#{sort}")
      expire_fragment("question-instance-index-#{record.question_instance_id}-false-#{sort}")
    end
