...
class HTMLUtilsTest < ActiveSupport::TestCase
  describe HTMLUtils, :process do
    it "should remove comment nodes from HTML" do
      comments = "<!-- foo --><!-- bar -->"
      html = "#{comments}<div>foobar</div>"
      assert_equal html.sub(comments, ""), HTMLUtils.process(html)
    end
    ...
    it "should pull content out of specific tags and place it at the parent level" do
      ["article",
       "section"].each do |tag|
        ...
        assert_equal after, HTMLUtils.process(before)
      end
    end
    ...
    it "should not wrap block level elements" do
      html = "<div>foobar</div>"
      assert_equal html, HTMLUtils.process(html)
    end
