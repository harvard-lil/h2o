...
def main(input_path, output_path):
    ...
    """
     Below is copied and pasted with zero improvements from my repl. I see no reason to improve it but it's obviously
     improvable.
    """
...
if __name__ == '__main__':
    ...
    if (not output_path.endswith('.md') and output_path != 'print') or not input_path.endswith('.docx'):
        print(f"Input path is {input_path} (needs to be *.docx) and output path is {output_path} (needs to be *.md or print.")
        print("{} {} Get your life together.".format(
            f"Does {input_path} look like a docx. No!" if not input_path.endswith('.docx') else "",
            f"Does {output_path} look like an md file or print? No!" if not output_path.endswith('.md') and output_path != 'print' else "",
        ))
        sys.exit(1)
    prompt = f"print the markdown hierarchy from {input_path} for styles to {output_path if output_path.endswith('md') else 'print'}?"
    if not pbwf.pbwf(question=prompt):
        sys.exit(1)
