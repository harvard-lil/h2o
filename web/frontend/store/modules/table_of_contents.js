...
const helpers = {
  addCSSClass: (cls) => {
    return (node) => {
      node.cssClasses = _.get(node, "cssClasses", [])
        .filter((x) => x !== cls)
        .concat([cls]);
      return node;
    };
  ...
  pruneFilter: (tree, filterFn) => {
    ...
    function dive(node) {
      return !filterFn(node)
        ? []
        : [node].concat(_.flatMap(node.children || [], dive));
    }
    return dive(tree);
  },
  firstMatching: (tree, pred) => {
    function dive(node) {
      return pred(node)
        ? node
        : _.find(_.flatMap(node.children || [], dive).filter(_.identity), pred);
    }
    return dive(tree);
  },
  findPath: (tree, pred) => {
    function dive(node, path) {
      ...
      return pred(node)
        ? [{ path: nextPath }]
        : _.flatMap(node.children || [], (x) => dive(x, nextPath));
    }
    return _.get(dive(tree, []), "0.path", []);
  },
  resourcePath: (casebook, subsection) => {
    return !subsection
      ? `/casebook/${casebook}/toc`
      : `/casebook/${casebook}/toc/${subsection}`;
  },
  findNode: (toc, id) => {
    const matchingNode = (node) => node.id === id;
    let trees = _.values(toc).map((tree) =>
      helpers.firstMatching(tree, matchingNode)
    );
  ...
  collapsedIDs: (toc, casebook) => {
    ...
    return helpers
      .flatFilter(toc[casebook], isCollapsed)
      .map((node) => node.id);
  },
...
const getters = {
  ...
  collapsedNodes: (state) => (casebook) =>
    helpers.collapsedIDs(state.toc, casebook),
  openNodes: (state) => (casebook) => helpers.openIDs(state.toc, casebook),
};

const collapseNode = (node) =>
  helpers.addCSSClass("collapsed")(helpers.addFlag("collapsed")(node));
const expandNode = (node) =>
  helpers.removeCSSClass("collapsed")(helpers.removeFlag("collapsed")(node));
//const setLoading = helpers.addFlag('loading');
const setLoaded = (node) =>
  helpers.removeCSSClass("loading")(helpers.addCSSClass("loaded")(node));
...
const actions = {
  ...
  toggleCollapsed: ({ commit }, { id }) => {
    commit("modifyAugment", {
      id,
      modifyFn: (node) =>
        _.get(node, "collapsed", false) ? expandNode(node) : collapseNode(node),
    });
  },
  ...
  setAudit: ({ commit, state }, { id }) => {
    let currentAudits = _.keys(state.augments)
      .filter((k) => _.get(state.augments[k], "audit", false))
      .map(parseInt);
  ...
  fetch: ({ commit }, { casebook, subsection }) => {
    ...
    Axios.get(helpers.resourcePath(casebook, subsection)).then((resp) => {
      if (!resp.data.id) {
  ...
  deleteNode: (
    ...
    return Axios.delete(helpers.resourcePath(casebook, targetId)).then(
      () => {
  ...
  moveNode: (
    { commit, state, dispatch },
    { casebook, rootNode, moverId, parent, index }
  ) => {
    let data = { parent, index };
    if (parent === null && rootNode != casebook) {
      data["parent"] = rootNode;
    }
    return Axios.patch(helpers.resourcePath(casebook, moverId), data).then(
      (resp) => {
        if (!resp.data.id) {
          resp.data.id = casebook;
        }
