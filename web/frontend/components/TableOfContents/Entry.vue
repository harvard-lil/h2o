<template>
    ...
    >
        <component
            :is="editing ? 'vue-nestable-handle' : 'div'"
            v-bind:item="item"
        >
            <div
                v-bind:class="{
                    listing: true,
                    resource: true,
                    temporary: item.resource_type == 'Temp',
                    editing: 'editing',
                }"
                v-if="isResource"
            >
                <div class="list-left">
                    ...
                    <div class="resource-container" v-if="isLegalDoc">
                        <a
                            :href="url"
                            class="section-title case-section-title"
                            >{{ item.title }}</a
                        >
                    ...
                    <div class="resource-container" v-else>
                        <a :href="url" class="section-title">{{
                            item.title
                        }}</a>
                    </div>
                </div>
                            ...
                            <div class="action-align-resource">
                                ...
                                >
                                    ...
                                    {{
                                        ...
                                        item.resource_type !== "Section"
                                            ? ""
                                            : "section and all contents"
                                    }}
                                </button>
                                <button
            ...
            <div
                ...
                v-bind:class="[
                    'listing',
                    'section',
                    item.children.length > 0 ? 'child-present' : 'child-free',
                    editing ? 'editing' : '',
                ]"
                v-else
            >
                ...
                <div class="list-right">
                    <div
                        class="resource-type-container"
                        v-if="item.is_transmutable && editing"
                    >
                        <entry-transmuter :item="item"></entry-transmuter>
                    </div>
                    <div v-else>&nbsp;</div>
        ...
        <div
            class="audit-drawer"
            v-if="item.audit && item.resource_type === 'Temp'"
        >
...
export default {
    ...
    computed: {
        ...
        autoSpecifyCandidate: function () {
            if (this.needsToSpecifyCase) {
                let lineGuess = pp.guessLineType(
                    this.item.title,
                    this.getSources()
                );
        ...
        dimmed: function () {
            return !this.item.audit &&
                this.$store.getters["globals/inAuditMode"]()
                ? "dimmed"
                : "";
        },
        animated: function () {
    ...
    methods: {
        auditThisCase: function () {
            this.$store.dispatch("table_of_contents/setAudit", {
                id: this.item.id,
            });
        },
        toggleSectionExpanded: function () {
            this.$store.dispatch("table_of_contents/toggleCollapsed", {
                id: this.item.id,
            });
        },
