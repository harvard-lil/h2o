...
def test_change_password(user, client):
    ...
    # visit form
    check_response(
        client.get(reverse("password_change")), content_includes=["Change your password"]
    )
...
def test_forgot_password(user, client, mailoutbox):
    ...
    # request reset email
    check_response(
        client.get(reverse("password_reset")), content_includes=["Forgotten your password?"]
    )
    check_response(
        client.post(reverse("password_reset"), {"email": user.email_address}, follow=True),
