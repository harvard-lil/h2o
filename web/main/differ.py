...
def assert_offsets_adjusted(before_str, after_str):
    ...
    assert (
        adjusted_str == after_str
    ), f"{before_str} was adjusted to {adjusted_str}, not {after_str}"
...
class AnnotationUpdater:
    ...
    def get_first_delta_offset(self):
        ...
        return next(
            (
                offset
                for offset, delta in zip(self.offsets, self.deltas)
                if type(delta) is AbsoluteDelta or delta != 0
            ),
            -1,
        )
