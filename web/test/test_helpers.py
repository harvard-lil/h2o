...
def check_response(
    response, status_code=200, content_type=_default, content_includes=None, content_excludes=None
):
...
def dump_casebook_outline(casebook):
    ...
    for node in casebook.contents.prefetch_resources().prefetch_related("annotations"):
        ...
        if node_type == "section":
            ...
            out.append(
                f"{indent}ContentNode<{node.id}> -> {type(resource).__name__}<{resource.id}>: {resource.name}"
            )
            for annotation in node.annotations.order_by("global_start_offset", "id"):
                out.append(
                    f"{indent} ContentAnnotation<{annotation.id}>: {annotation.kind} {annotation.global_start_offset}-{annotation.global_end_offset}"
                )
    return out
...
def _dump_content_tree(node):
    return [
        [child, child.content_tree__parent, _dump_content_tree(child)]
        for child in node.content_tree__children
    ]
